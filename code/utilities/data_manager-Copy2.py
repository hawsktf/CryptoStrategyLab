...
from config.config import EXCHANGES, TIMEFRAMES, BASE_DATA_PATH, ALPACA_API_KEY, ALPACA_SECRET_KEY
from alpaca_trade_api.rest import REST, TimeFrame

alpaca_rest = REST(ALPACA_API_KEY, ALPACA_SECRET_KEY, base_url="https://paper-api.alpaca.markets")

class DataManager:
    def __init__(self, name: str, path: str = "../data") -> None:
        self.name = name
        self.path = Path(__file__).parent.joinpath(BASE_DATA_PATH).resolve()
        if name != 'alpaca':
    ...
    def _check_support(self) -> None:
        if self.name != 'alpaca' and self.name not in EXCHANGES:
    ...
    def download(self, symbol: str, timeframe: str, start_date: Optional[str] = None,
        ...
        if self.name == 'alpaca':
            ...
            bars['date'] = pd.to_datetime(bars['date'], utc=True).dt.floor('D').dt.tz_localize(None)
